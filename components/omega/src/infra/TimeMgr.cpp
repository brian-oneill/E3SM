...
bool TimeInterval::isDivisibleBy(
   ...
   if ((IsCalendar && Units == TimeUnits::Months) || 
      ...
      bool HasVariableMonths = (CalKind == CalendarGregorian || 
                                CalKind == CalendarNoLeap || 
                                CalKind == CalendarJulian);
      
       if (HasVariableMonths) {
          // For variable-month calendars, check if comparing month with day-based or shorter units
          bool ComparingMonthWithDayOrShorter = 
             (IsCalendar && Units == TimeUnits::Months && 
              (Divisor.IsCalendar && (Divisor.Units == TimeUnits::Days || 
               ...
               !Divisor.IsCalendar)) ||
             (Divisor.IsCalendar && Divisor.Units == TimeUnits::Months && 
              (IsCalendar && (Units == TimeUnits::Days || 
          ...
          if (ComparingMonthWithDayOrShorter) {
             ...
             if (Divisor.IsCalendar && Divisor.Units == TimeUnits::Months) {
                 ...
                 // For variable-month calendars, day-based intervals cannot be evenly divided by months
                 // (since months have variable length)
                 return false;
              } else if (Divisor.IsCalendar && Divisor.Units == TimeUnits::Days) {
                // Divisor is in days - only 1 day divides all variable-length months
                return (Divisor.CalInterval == 1);
             } else if (Divisor.IsCalendar && (Divisor.Units == TimeUnits::Hours || 
                                              Divisor.Units == TimeUnits::Minutes || 
                                              Divisor.Units == TimeUnits::Seconds)) {
                // Divisor is calendar-based hours/minutes/seconds - convert to seconds
                I8 DivisorSec = Divisor.CalInterval * Calendar::OmegaCal->SecondsPerDay;
   ...
   if (IsCalendar) {
      ...
      case TimeUnits::Years:
         ...
         break;
      case TimeUnits::Months:
         ThisInSeconds = CalInterval * (Calendar::OmegaCal->SecondsPerYear / MONTHS_PER_YEAR);
         break;
      case TimeUnits::Days:
   ...
   if (Divisor.IsCalendar) {
      ...
      case TimeUnits::Years:
         DivisorInSeconds = Divisor.CalInterval * Calendar::OmegaCal->SecondsPerYear;
         break;
      case TimeUnits::Months:
         DivisorInSeconds = Divisor.CalInterval * (Calendar::OmegaCal->SecondsPerYear / MONTHS_PER_YEAR);
         break;
      case TimeUnits::Days:
         DivisorInSeconds = Divisor.CalInterval * Calendar::OmegaCal->SecondsPerDay;
         break;
      default:
         ABORT_ERROR("TimeInterval::isDivisibleBy: Unknown calendar units");
      }
