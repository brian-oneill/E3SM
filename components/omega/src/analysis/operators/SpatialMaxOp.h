...
class SpatialMaxOp : public AnalysisOperator {
 public:
   ...
   SpatialMaxOp(const std::vector<std::string> &UpstreamNames, ///< [in] input field names
 ...
 private:
   ...
   /// Output data array holding the computed spatial maximum (single scalar value)
   typename Array1D<ScalarT>::type OutputData;

   /// Temporary storage for the computed maximum value before copying to OutputData
   ScalarT SpatialMax;
