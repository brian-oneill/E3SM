               ...
               for (I4 L = IRange[6]; L <= IRange[7]; ++L) {
                  for (I4 M = IRange[8]; M <= IRange[9]; ++M) {
                     size_t LinearAdd = I * Strides[0] + J * Strides[1] +
                                        K * Strides[2] + L * Strides[3] +
                                        M * Strides[4];
                     IT MaskVal = Arr2.data()[LinearAdd];
                     if (MaskVal != 0) {
                        IT TestVal =
                            Arr1.data()[LinearAdd] * MaskVal;
                        if (TestVal < LocalMin)
                           LocalMin = TestVal;
                     }
                  }
          ...
          KOKKOS_LAMBDA(int I, int J, int K, int L, int M, IT &DevMin) {
             if (I >= LocRange(0) and J >= LocRange(2) and K >= LocRange(4) and
                ...
                size_t LinearAdd = I * LocStrides(0) + J * LocStrides(1) +
                                   K * LocStrides(2) + L * LocStrides(3) +
                                   M * LocStrides(4);
                IT MaskVal = LocArr2.data()[LinearAdd];
                if (MaskVal != 0) {
                   IT TestVal =
                       LocArr1.data()[LinearAdd] * MaskVal;
                   DevMin = Kokkos::min(TestVal, DevMin);
                }
             }
          },
...
localMinVal(const Kokkos::View<T, ML, MS> Arr1, ///< [in] 1st array in product
            const MPI_Comm Comm,                ///< [in] MPI Communicator
            const Array2DReal Arr2, ///< [in] 2nd array in product
            IT &LocalMinVal,                    ///< [out] local min
            const std::vector<I4> *IndxRange = nullptr ///< [in] index range
               ...
               for (I4 L = IRange[6]; L <= IRange[7]; ++L) {
                  for (I4 M = IRange[8]; M <= IRange[9]; ++M) {
                     size_t LinearAdd = I * Strides[0] + J * Strides[1] +
                                        K * Strides[2] + L * Strides[3] +
                                        M * Strides[4];
                     size_t LinearAdd2 = L * Strides[3] + M * Strides[4];
                     IT MaskVal = Arr2.data()[LinearAdd2];
                     if (MaskVal != 0) {
                        IT TestVal =
                            Arr1.data()[LinearAdd] * MaskVal;
                        if (TestVal < LocalMin)
          ...
          KOKKOS_LAMBDA(int I, int J, int K, int L, int M, IT &DevMin) {
             if (I >= LocRange(0) and J >= LocRange(2) and K >= LocRange(4) and
                ...
                size_t LinearAdd = I * LocStrides(0) + J * LocStrides(1) +
                                   K * LocStrides(2) + L * LocStrides(3) +
                                   M * LocStrides(4);
                size_t LinearAdd2 = L * LocStrides[3] + M * LocStrides[4];
                IT MaskVal = LocArr2.data()[LinearAdd2];
                if (MaskVal != 0) {
                   IT TestVal =
                       LocArr1.data()[LinearAdd] * MaskVal;
                   DevMin = Kokkos::min(TestVal, DevMin);
...
globalMinVal(const Kokkos::View<T, ML, MS> Arr1, ///< [in] 1st array in product
   ...
   // Call routine to find local min (also performs error checks)
   I4 LocalMin;
   localMinVal(Arr1, Comm, Arr2, LocalMin, IndxRange);
...
globalMaskedSum(const Kokkos::View<T1, ML1, MS1> Arr1,
   ...
   int Err =
       MPI_Allreduce(&LocalSum, &GlobalSum, 1, MPI_INT32_T, MPI_SUM, Comm);
   if (Err != MPI_SUCCESS)
      ABORT_ERROR("globalMaskedSum (I4 array product): Error in MPI_Allreduce");
   return GlobalSum;
}
...
std::enable_if_t<
    ...
    std::is_arithmetic_v<typename Kokkos::View<T2, ML2, MS2>::value_type>,
    void>
localMaskedMin(const Kokkos::View<T1, ML1, MS1> Arr1, ///< [in] 1st array
...
std::enable_if_t<
    ...
    std::is_arithmetic_v<typename Kokkos::View<T2, ML2, MS2>::value_type>,
    R8>
globalMaskedMin(const Kokkos::View<T1, ML1, MS1> Arr1, ///< [in] 1st array
                   const Kokkos::View<T2, ML2, MS2> Arr2, ///< [in] mask/weight (1D or 2D)
                   const MPI_Comm Comm,                    ///< [in] MPI Communicator
                   const std::vector<I4> *IndxRange = nullptr ///< [in] index range
) {
...
///-----------------------------------------------------------------------------
/// Finds the local maximum of a Kokkos array product with masking
template <typename T1, typename ML1, typename MS1, typename IT,
          typename T2, typename ML2, typename MS2>
std::enable_if_t<
    std::is_same_v<IT, typename Kokkos::View<T1, ML1, MS1>::value_type> &&
...
localMaskedMax(const Kokkos::View<T1, ML1, MS1> Arr1, ///< [in] 1st array
   ...
   // Error checks
   OMEGA_REQUIRE(Arr2.rank == 1 || Arr2.rank == 2,
                 "localMaxBroadcast: Arr2 must be 1D or 2D");
   OMEGA_REQUIRE(Arr1.rank >= Arr2.rank,
                 "localMaxBroadcast: Arr1 rank must be >= Arr2 rank");

   // Verify dimension matching
   if (Arr2.rank == 1) {
       int horizDim1 = (Arr1.rank == 1) ? 0 : Arr1.rank - 2;
       OMEGA_REQUIRE(Arr1.extent(horizDim1) == Arr2.extent(0),
                     "localMaxBroadcast: Horizontal dimensions must match");
   } else { // Arr2.rank == 2
       OMEGA_REQUIRE(Arr1.rank >= 2,
                     "localMaxBroadcast: Arr1 must be at least 2D when Arr2 is 2D");
       OMEGA_REQUIRE(Arr1.extent(Arr1.rank - 2) == Arr2.extent(0),
                     "localMaxBroadcast: Horizontal dimensions must match");
       OMEGA_REQUIRE(Arr1.extent(Arr1.rank - 1) == Arr2.extent(1),
                     "localMaxBroadcast: Vertical dimensions must match");
   }
...
globalMaskedMax(const Kokkos::View<T1, ML1, MS1> Arr1, ///< [in] 1st array
   ...
   int Err = MPI_Allreduce(&LocalMax, &GlobalMax, 1, MPI_DOUBLE, MPI_MAX, Comm);
   if (Err != MPI_SUCCESS)
      ABORT_ERROR("globalMaxBroadcast: Error in MPI_Allreduce");
...
globalMaskedMax(const Kokkos::View<T1, ML1, MS1> Arr1, ///< [in] 1st array
   ...
   // Compute final maximum across MPI tasks
   I4 GlobalMax;
   int Err = MPI_Allreduce(&LocalMax, &GlobalMax, 1, MPI_INT32_T, MPI_MAX, Comm);
   if (Err != MPI_SUCCESS)
      ABORT_ERROR("globalMaxBroadcast: Error in MPI_Allreduce");
...
globalMaskedMax(const Kokkos::View<T1, ML1, MS1> Arr1, ///< [in] 1st array
   ...
   // Compute final maximum across MPI tasks
   I8 GlobalMax;
   int Err = MPI_Allreduce(&LocalMax, &GlobalMax, 1, MPI_INT64_T, MPI_MAX, Comm);
   if (Err != MPI_SUCCESS)
      ABORT_ERROR("globalMaxBroadcast: Error in MPI_Allreduce");
