...
//------------------------------------------------------------------------------
// Helper to get dimensions based on rank
std::vector<I4> getDimsForRank(int Rank, const HorzMesh *Mesh, 
...
//------------------------------------------------------------------------------
// Helper to get dimension names
std::vector<std::string> getDimNamesForRank(int Rank) {
   if (Rank == 1) {
      return {"Dim0"};
   } else if (Rank == 2) {
...
//------------------------------------------------------------------------------
// Generic createTestField template for 1D arrays
template<typename ArrayType>
typename std::enable_if<ArrayType::rank == 1, void>::type
createTestField(const std::string &FieldName,
                const std::vector<I4> &Dims,
                std::function<typename ArrayType::non_const_value_type(I4)> ValueFunc) {
   
   using ScalarT = typename ArrayType::non_const_value_type;
   
   // Create field
   auto DimNames = getDimNamesForRank(1);
...
//------------------------------------------------------------------------------
// Generic createTestField template for 2D arrays
template<typename ArrayType>
typename std::enable_if<ArrayType::rank == 2, void>::type
createTestField(const std::string &FieldName,
                const std::vector<I4> &Dims,
                std::function<typename ArrayType::non_const_value_type(I4, I4)> ValueFunc) {
   
   using ScalarT = typename ArrayType::non_const_value_type;
   
   // Create field
   auto DimNames = getDimNamesForRank(2);
   auto TestField = Field::create(
      FieldName,
      "Test field for multi-type validation",
      "m",
...
//------------------------------------------------------------------------------
// Generic createTestField template for 3D arrays
template<typename ArrayType>
typename std::enable_if<ArrayType::rank == 3, void>::type
createTestField(const std::string &FieldName,
                const std::vector<I4> &Dims,
                std::function<typename ArrayType::non_const_value_type(I4, I4, I4)> ValueFunc) {
   
   using ScalarT = typename ArrayType::non_const_value_type;
   
   // Create field
   auto DimNames = getDimNamesForRank(3);
   auto TestField = Field::create(
      FieldName,
      "Test field for multi-type validation",
      "m",
...
void testSpatialMaxOp_Type(const std::string &TypeName,
   ...
   std::vector<I4> Dims = getDimsForRank(Rank, Mesh, VCoord);
   std::string FieldName = "TestFieldMax_" + TypeName;
   
   // Create test field with values that sum indices
   if constexpr (Rank == 1) {
      createTestField<ArrayType>(FieldName, Dims,
         [](I4 i) -> ScalarT {
   ...
   // Verify
   bool Passed = (std::abs(ComputedMax - ExpectedMaxReal) <= getTolerance<Real>());
...
void testSpatialMinOp_Type(const std::string &TypeName,
   ...
   // Verify
   bool Passed = (std::abs(ComputedMin - ExpectedMinReal) <= getTolerance<Real>());
...
void testSpatialMeanOp_Type(const std::string &TypeName,
   ...
   // Verify
   bool Passed = (std::abs(ComputedMean - ExpectedMean) <= getTolerance<Real>());
...
void testSpatialStdDevOp_Type(const std::string &TypeName,
   ...
   // Verify
   bool Passed = (std::abs(ComputedStdDev - ExpectedStdDev) <= getTolerance<Real>());
...
void testTimeMeanOp_Type(const std::string &TypeName,
   ...
   if constexpr (Rank == 1) {
      ...
      for (I4 i = 0; i < std::min(10, Dims[0]); ++i) {
         if (std::abs(ResultHost(i) - ExpectedMean) > getTolerance<Real>()) {
            Passed = false;
            break;
         }
      }
      ...
      for (I4 i = 0; i < std::min(5, Dims[0]); ++i) {
         for (I4 j = 0; j < std::min(5, Dims[1]); ++j) {
            if (std::abs(ResultHost(i, j) - ExpectedMean) > getTolerance<Real>()) {
               Passed = false;
               break;
            }
         }
      ...
      for (I4 i = 0; i < std::min(3, Dims[0]); ++i) {
         for (I4 j = 0; j < std::min(3, Dims[1]); ++j) {
            for (I4 k = 0; k < std::min(3, Dims[2]); ++k) {
               if (std::abs(ResultHost(i, j, k) - ExpectedMean) > getTolerance<Real>()) {
                  Passed = false;
                  break;
               }
            }
