...
Operators must be able to produce multiple output fields. This allows, for
example, operators to simultaneously return separable components of a vector
field, or the components of a spatial gradient.
...
in the chain, and only the final operator $\mathcal{O}_m$ writes to the
output stream.
...
2. **For** each analysis field $a \in \mathcal{A}$:
    - Parse string into chain of operators: $\{\mathcal{O}_1, \ldots,
      \mathcal{O}_m\} \leftarrow \texttt{Analysis::parseOperatorChain}(a)$
    - **For** $i = 1$ to $m$:
...
At operator creation time, the factory inspects the first upstream Field's
...
**Output**: `unique_ptr<AnalysisOperator>` to the correct typed specialization

1. Retrieve the first upstream Field from the Field registry
...
- **Discrete-sampling (non-temporal-reduction) terminal operators**: borrow a
  raw pointer to the write alarm of their associated output stream. The
  stream owns this alarm.
- **Temporal reduction terminal operators**: require two alarms. An
  **accumulation alarm** controls how frequently a sample is added to the
  ...
  as `vector<unique_ptr<Alarm>> AccumulationAlarms`; their raw pointers are
  stored in `ComputeAlarms` alongside the borrowed output alarm pointer.
...
`AnalysisGroup`. The orchestrator iterates over these nodes during
...
frequency and temporal reduction characteristics.
...
class AnalysisOperator {
 public:
   ...
   ~AnalysisOperator(); // destroys owned output Fields
...
Operators with a non-empty `StreamNames` are terminal nodes whose output is
written to one or more output streams. Operators with an empty `StreamNames`
are intermediate nodes computed on demand when a downstream alarm rings.
...
`AnalysisManager->parseChainAndBuildOps()`. For each
